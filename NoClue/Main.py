...
add a message obj to be printed out and so if stament can be inhariated  ok
"""
...
ms2 = mM.message( "",name, "grew an arm" )
...
#this makes the "if stament" in the magic class inhariable


# magic should be thought of pure uncontroled energy (unlike spells )
# cost should be between 1 - 100 on a min to max
class magic:
...
class fireSpell (magic):
    def __init__(self, name,power,time):
        super().__init__(power,time)
        self.name = name
        self.mSet = mSet2
...
rock = nounMod.noun("rock", 3)
#zprint(rock.getName())
test = magic(50,4)
test2 = fireSpell("fire",50,4)
test.use(rock)
test2.use(rock)
